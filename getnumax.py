...
def get_linear_bg(frequency, power, numax_est, evol_stage, sm, lowerp, upperp):
    ...
    sig = (sm*2*(dnu_est/resolution))/np.sqrt(8.0*np.log(2.0))
    smps = convolve_fft(np.copy(power), Gaussian1DKernel(int(sig)))
...
def ps_no_slope(frequency, power, time, numax_est, evol_stage, background_model, sm, lowerp, upperp, w_noise, verbose):
    ...
    if background_model == 'nuSYD':
        ...
        pssm = ps_smooth(frequency, power, numax_est, evol_stage, sm)

        bg = model2

        power_slope_removed = pssm - bg   ### WHY????
...
def plot_power_excess(ax, frequency, power, pssm, numax_est, numax_measured, ps_mask, ylim_offset, width_idx):
    ...
    axin.plot(mask_frequency, mask_pssm, c = 'purple', alpha = 0.8, zorder = 1)


    axin.axvline(numax_measured, color = 'purple', ls = 'dashed', lw = 3, zorder = 5)
    ...
    xticks = np.arange(int(ps_mask[0]), int(ps_mask[1]+1), 2)
...
def plot_PS(scale, frequency, power, pssm, bg, numax_measured, numax_est, ps_mask, star_id, peak_measured, width_idx, width_measured, save, ylim_offset, background_model):
    ...
    if background_model == 'linear' or background_model == 'harvey':
        ...
        ax.plot(frequency, pssm+bg, c = 'purple', alpha = 1, label = 'Smoothed ps + bg', zorder = 1, lw = 2);
    else:
    ...
    if scale == 'linear':
        plot_power_excess(ax, frequency, power, pssm, numax_est, numax_measured, ps_mask, ylim_offset, width_idx)
...
def main(frequency, power, time, numax_est, star_id, evol_stage, background_model = 'nuSYD', verbose = False, plot = False, save = False, ylim_offset = 0, testing = False, lowerp = None, upperp = None, sm = None):
    ...
    power_no_wnoise, w_noise = ps_no_wnoise(frequency, power, star_id, verbose)

    # print(f'Using the {background_model} background model')
    power_slope_removed, bg = ps_no_slope(frequency, power_no_wnoise, time, numax_est, evol_stage, background_model, sm, lowerp, upperp, w_noise, verbose)
    ...
    if background_model == 'nuSYD' or background_model == 'white':
        ...
        pssm = ps_smooth(frequency, power_slope_removed, numax_est, evol_stage, sm)
    else:
