...
def plot_PS(scale, frequency, power, pssm, bg, numax_measured, numax_est, ps_mask, star_id, peak_measured, width_idx, width_measured, save, ylim_offset, background_model):
    ...
    if background_model == 'linear' or background_model == 'harvey':
        ...
        ax.plot(frequency, pssm, c = 'purple', alpha = 1, label = 'Smoothed ps', zorder = 1, lw = 2);
    else:
...
def GetNumax(frequency, power, time, numax_est, star_id, evol_stage, background_model = 'nuSYD', verbose = False, plot = False, save = False, ylim_offset = 0, testing = False, lowerp = None, upperp = None, sm = None):
...
def GetNumaxUncertainty(frequency, power, time, params, star_id, evol_stage, background_model = 'nuSYD', mc_iters = 200, verbose = False, plot = False, save = False, nyquist = 277.78, lowerp = None, upperp = None, sm = None):
    ...
    while i < mc_iters:
        if i == 0:
            ...
            numax_temp, peak_temp, width_temp = GetNumax(frequency_cp, random_pow, time, numax, star_id, evol_stage, background_model, lowerp = lowerp, upperp = upperp, sm = sm)
